...
string Input::inputGroup(SOCKET& s) {
	...
	while (true) {
		...
		try {
			...
			bufSend += "Некорректный ввод, группа должна состоять из 6 цифр\n";
		}
	}
...
char Input::inputSymbol(SOCKET& s) {
	...
	vector<char> bufRecv(BUFF_SIZE);
	while (true) {
		bufSend += "Введите первую букву фамилии: ";
		length = strlen(bufSend.c_str());
		send(s, (char*)&length, sizeof(length), 0);
		send(s, bufSend.c_str(), length, 0);
		bufSend = "";
		try {
			...
			bufSend += "Некорректный ввод, ввод должен быть заглавной буквой русского алфавита\n";
		}
	}
